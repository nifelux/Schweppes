...
function validWebhookSignature(payload) {
  ...
  if (!signature || !identifier || amountRaw === null || !Number.isFinite(amount)) return false;

  // TargetGrowths signs the exact amount representation in the callback.
  // The captured callback uses "600.00000000", so do not normalize it before
  // calculating the HMAC. The formatted alternatives retain compatibility with
  // older callbacks that used two or zero decimal places.
